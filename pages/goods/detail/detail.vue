			...
			<view class="goods-container">
				...
				<view v-if="showDiscount && goodsSkuDetail.discountTimeMachine" class="goods-promotion" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/goods/detail_promotion_left_bg.png') + ')' }">
					<view class="price-info">
						<view class="img-wrap">
							<image :src="$util.img('upload/uniapp/goods/detail_discount_tag.png')" mode="aspectFit"></image>
						</view>
						...
						<uni-popup ref="couponPopup" type="bottom">
							<view class="goods-coupon-popup-layer popup-layer">
								...
								<scroll-view class="coupon-body" scroll-y>
									<view class="coupon-item ns-gradient-diy-goods-list" :class="{ disabled: item.is_lingqu != undefined }" v-for="(item, index) in couponList"
									 :key="index" :data-theme="themeStyle">
				...
				<ns-goods-sku ref="goodsSku"  :goods-detail="goodsSkuDetail" :max-buy="goodsSkuDetail.max_buy"
				@error="errorDo" @refresh="refreshGoodsSkuDetail"  :min-buy="goodsSkuDetail.min_buy"></ns-goods-sku>
		...
		<ns-goods-action :safeArea="isIphoneX" :isiPhone="isiPhone12">
			<template v-if="goodsSkuDetail.goods_state == 1 && goodsSkuDetail.verify_state == 1">
				...
				<block v-else-if="is_wholesaler==3">
					<ns-goods-action-button :class="['goods-action-button',isiPhone12?'iPhone12':'',goodsSkuDetail.is_virtual == 0 ? 'active1' : '']" text="加入购物车"
					 background="color-join-cart" @click="joinCart" v-if="goodsSkuDetail.is_virtual == 0 && goodsSkuDetail.photometric == 0 && goodsSkuDetail.luminosity_status != 1" />
					<ns-goods-action-button :class="['goods-action-button',isiPhone12?'iPhone12':'',goodsSkuDetail.is_virtual == 0 ? 'active2' : 'active4']"
					 text="立即购买" @click="buyNow" v-if="goodsSkuDetail.photometric == 0 && goodsSkuDetail.luminosity_status != 1" />
					 
					 <view v-if="goodsSkuDetail.photometric != 0 || goodsSkuDetail.luminosity_status == 1" class="joinCartBtn" style="display: flex;flex: 1;background: #FFB644;color: #FFFFFF;border-radius: 999rpx;
					 margin: 0 20rpx;justify-content: center;align-items: center;padding: 10rpx 0;"  @click="joinCart">加入购物车</view>
				</block>
				<block v-else>
					<ns-goods-action-button class="goods-action-button active4"   text="请先认证"   @click="buyNowWholesaler"/>
				</block>
			</template>
			<template v-else>
